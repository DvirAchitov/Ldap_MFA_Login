#!/bin/bash
# Script to add a new user to OpenLDAP with MFA support (Kubernetes version)
...
echo "============================================================"
echo "LDAP User Creation Tool (Kubernetes)"
echo "============================================================"
...
if [[ -z "$LDAP_POD" ]]; then
    echo "❌ ERROR! LDAP pod not found. Make sure LDAP is running."
    exit 1
fi
...
NEXT_UID=$(kubectl exec $LDAP_POD -- ldapsearch -x -H ldap://localhost \
...
# Add user to LDAP via kubectl exec
echo "$LDIF_CONTENT" | kubectl exec -i $LDAP_POD -- ldapadd -x \
